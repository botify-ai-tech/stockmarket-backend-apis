...
def scrap_daily1(share):
    session = SessionLocal()  # Moved here
    try:
        ...
        if existing_data:
            existing_data.market_cap = data.get('Market Cap')
            existing_data.pb_ratio = data.get('P/B')
            existing_data.enterprise_value = data.get('Enterprise Value')
            existing_data.pe_ratio = data.get('P/E')
            existing_data.dividend_yield = data.get('Div. Yield')
            existing_data.debt = data.get('DEBT')
            session.commit()
        ...
        session.rollback()
        logging.error(f"Database error for {share}: {str(e)}")
...
def parallel_scraper(start, end):
    shares_to_scrape = all_shares[start:end+1]
    unscraped_companies = []
...
if __name__ == '__main__':
    parallel_scraper(0,len(all_shares))